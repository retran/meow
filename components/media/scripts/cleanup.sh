...
ui_info "$(fmt "media_cleanup_running")"

if pgrep -f "OBS" >/dev/null; then
  ui_info "$(fmt "media_stopping_obs")"
  osascript -e 'tell application "OBS" to quit' 2>/dev/null || true
  sleep 2
fi

if command -v osascript >/dev/null 2>&1; then
  ui_info "$(fmt "media_removing_login_items")"
  osascript -e '
...
ui_info "$(fmt "media_cleaning_cache")"
...
ui_success "$(fmt "media_cleanup_completed")"
ui_info "$(fmt "media_user_recordings_preserved")"

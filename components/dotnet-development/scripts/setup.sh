...
if ! command -v dotnet >/dev/null 2>&1; then
  ui_warning "$(fmt "dotnet_not_found_skip")"
  exit 0
fi

ui_action_start "$(fmt "dotnet_configuring")"

ui_info "$(fmt "dotnet_configuring_tools_path")"
dotnet_tools_path="$HOME/.dotnet/tools"
mkdir -p "$dotnet_tools_path" 2>/dev/null || true

ui_info "$(fmt "dotnet_installing_global_tools")"
...
ui_action_success "$(fmt "dotnet_configured_successfully")"

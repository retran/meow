...
if [[ -d "$HOME/Library/Application Support/Telegram Desktop" ]]; then
  ui_info "$(fmt "personal_comm_cleaning_telegram_cache")"
...
if [[ -d "$HOME/Library/Caches/WhatsApp" ]]; then
  ui_info "$(fmt "personal_comm_cleaning_whatsapp_cache")"
  rm -rf "$HOME/Library/Caches/WhatsApp" 2>/dev/null || true
fi

ui_info "$(fmt "personal_comm_removing_login_items")"
osascript -e '
...
ui_success "$(fmt "personal_comm_cleanup_completed")"

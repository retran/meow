...
ui_info "$(fmt "kotlin_dev_cleanup_running")"

if pgrep -f "IntelliJ IDEA" >/dev/null; then
  ui_info "$(fmt "kotlin_dev_stopping_intellij")"
  osascript -e 'tell application "IntelliJ IDEA" to quit' 2>/dev/null || true
  sleep 3
fi

if [[ -d "$HOME/.gradle" ]]; then
  ui_info "$(fmt "kotlin_dev_cleaning_gradle_cache")"

...
ui_info "$(fmt "core_dev_cleanup_running")"

if command -v git-lfs >/dev/null 2>&1; then
  ui_info "$(fmt "core_dev_cleaning_git_lfs_cache")"
  git lfs prune 2>/dev/null || true
fi

if [[ -d "$HOME/.config/lazygit" ]]; then
  ui_info "$(fmt "core_dev_cleaning_lazygit_cache")"
  rm -rf "$HOME/.config/lazygit/logs" 2>/dev/null || true
fi

if command -v gh >/dev/null 2>&1; then
  ui_info "$(fmt "core_dev_cleaning_gh_cache")"
  gh cache delete --all 2>/dev/null || true
fi

if [[ -d "$HOME/.task" ]]; then
  ui_info "$(fmt "core_dev_cleaning_task_cache")"
  rm -rf "$HOME/.task/cache" 2>/dev/null || true
fi

ui_info "$(fmt "core_dev_cleaning_temp_files")"
...
ui_success "$(fmt "core_dev_cleanup_completed")"

...
ui_info "$(fmt "desktop_essential_cleanup_running")"

if [[ -d "$HOME/Library/Caches/Google/Chrome" ]]; then
  ui_info "$(fmt "desktop_essential_cleaning_chrome_cache")"
...
if [[ -d "$HOME/Library/Caches/com.microsoft.VSCode" ]]; then
  ui_info "$(fmt "desktop_essential_cleaning_vscode_cache")"

...
ui_info "TODO: write message - desktop_essential_cleanup_running"

if [[ -d "$HOME/Library/Caches/Google/Chrome" ]]; then
  ui_info "TODO: write message - desktop_essential_cleaning_chrome_cache"
...
if [[ -d "$HOME/Library/Caches/com.microsoft.VSCode" ]]; then
  ui_info "TODO: write message - desktop_essential_cleaning_vscode_cache"
...
if [[ -d "$HOME/Library/Caches/com.raycast.macos" ]]; then
  ui_info "TODO: write message - desktop_essential_cleaning_raycast_cache"
  rm -rf "$HOME/Library/Caches/com.raycast.macos" 2>/dev/null || true
fi

if [[ -d "$HOME/Library/Logs/Alacritty" ]]; then
  ui_info "TODO: write message - desktop_essential_cleaning_alacritty_logs"
  rm -rf "$HOME/Library/Logs/Alacritty" 2>/dev/null || true
fi

if [[ -d "$HOME/Library/Caches/com.bitwarden.desktop" ]]; then
  ui_info "TODO: write message - desktop_essential_cleaning_bitwarden_cache"
  rm -rf "$HOME/Library/Caches/com.bitwarden.desktop" 2>/dev/null || true
fi

ui_success "TODO: write message - desktop_essential_cleanup_completed"

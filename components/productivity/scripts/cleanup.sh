...
if [[ -d "$HOME/Library/Caches/com.notion.NotionCalendar" ]]; then
  ui_info "$(get_static_message "productivity_cleaning_notion_calendar_cache")"
  rm -rf "$HOME/Library/Caches/com.notion.NotionCalendar" 2>/dev/null || true
fi

if [[ -d "$HOME/Library/Caches/com.amazon.Kindle" ]]; then
  ui_info "$(get_static_message "productivity_cleaning_kindle_cache")"
  rm -rf "$HOME/Library/Caches/com.amazon.Kindle" 2>/dev/null || true
fi

ui_info "$(get_static_message "productivity_removing_login_items")"
osascript -e '
...
ui_success "$(get_static_message "productivity_cleanup_completed")"

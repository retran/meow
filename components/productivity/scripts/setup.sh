...
if [[ -d "/Applications/Notion Calendar.app" ]]; then
  ui_info "$(fmt "productivity_configuring_notion_calendar")"
...
if [[ -d "$kindle_app_path" ]]; then
  ui_info "$(fmt "productivity_configuring_kindle")"

  defaults write com.amazon.Kindle WhisperSyncEnabled -bool true 2>/dev/null || true
fi

ui_action_success "$(fmt "productivity_configured_successfully")"

...
ui_info "$(fmt "rust_dev_cleanup_running")"

if command -v cargo >/dev/null 2>&1; then
  ui_info "$(fmt "rust_dev_cleaning_cargo_cache")"
  cargo cache --autoclean 2>/dev/null || true

  ui_info "$(fmt "rust_dev_cleaning_registry_cache")"
  if [[ -d "$HOME/.cargo/registry" ]]; then
    rm -rf "$HOME/.cargo/registry/cache" 2>/dev/null || true
  fi

  ui_info "$(fmt "rust_dev_cleaning_git_cache")"
  if [[ -d "$HOME/.cargo/git" ]]; then
    rm -rf "$HOME/.cargo/git/checkouts" 2>/dev/null || true
  fi
fi

ui_info "$(fmt "rust_dev_cleaning_target_dirs")"
...
if [[ -d "$HOME/.rustup/tmp" ]]; then
  ui_info "$(fmt "rust_dev_cleaning_rustup_temp")"
  rm -rf "$HOME/.rustup/tmp" 2>/dev/null || true
fi

ui_success "$(fmt "rust_dev_cleanup_completed")"

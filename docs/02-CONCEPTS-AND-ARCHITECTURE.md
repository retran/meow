...
To understand how these layers work together, let's trace the execution of a [`meowctl install den-personal`](./03-COMMAND-REFERENCE.md#meowctl-install) command.

1.  **Parsing YAML**:
    -   `meowctl` calls the `install_preset` function in `lib/presets/presets.sh`.
    -   This function first reads the [`presets/den-personal/preset.yaml`](./06-PRESETS-AND-ROLES.md#composition-creating-your-own-presets) file.
...
2.  **Resolving Dependencies**:
    -   For each component required by the preset, the system reads its [`components/.../component.yaml`](./05-COMPONENT-DEVELOPMENT.md#componentyaml) file.
...
4.  **Installing System Packages**:
    ...
    -   Packages listed in the component's [`packages/apt.list`](./05-COMPONENT-DEVELOPMENT.md#packageslist) file are then installed.

5.  **Running Setup Scripts**:
    -   After installing packages, the component's lifecycle scripts are executed in order:
        -   [`scripts/preinstall.sh`](./05-COMPONENT-DEVELOPMENT.md#installation-time-scripts) (if it exists)
        -   [`scripts/setup.sh`](./05-COMPONENT-DEVELOPMENT.md#installation-time-scripts) (if it exists and the component is being installed for the first time)

6.  **Linking Dotfiles**:
    -   Finally, the symlink logic in `lib/symlinks/symlinks.sh` is invoked.
    -   It reads the YAML files in the component's [`symlinks/`](./05-COMPONENT-DEVELOPMENT.md#symlinksyaml) directory.
...
➡️ **[Learn How to Create and Structure Components](./05-COMPONENT-DEVELOPMENT.md)**

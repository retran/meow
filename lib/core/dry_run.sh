...
dry_run_command() {
  ...
  if is_dry_run; then
    dry_run_ui_info "$(_f "TODO: write message - dry_run_would_execute" "$description")"
    return 0
  else
    "$@"
  fi
...
dry_run_command_info() {
  local command="$1"
  echo -e "  ${CYAN}[DRY-RUN]${RESET}   $(_f "TODO: write message - dry_run_command" "${NORMAL}$command${RESET}")"
}
...
dry_run_file_operation() {
  ...
  if is_dry_run; then
    case "$operation" in
      "create_symlink")
        dry_run_ui_info "$(_f "TODO: write message - dry_run_create_symlink" "$target" "$source")"
        ;;
      "create_dir")
        dry_run_ui_info "$(_f "TODO: write message - dry_run_create_directory" "$target")"
        ;;
      "remove_file")
        dry_run_ui_info "$(_f "TODO: write message - dry_run_remove_file" "$target")"
        ;;
      "remove_dir")
        dry_run_ui_info "$(_f "TODO: write message - dry_run_remove_directory" "$target")"
        ;;
      "backup_file")
        dry_run_ui_info "$(_f "TODO: write message - dry_run_backup_file" "$target")"
        ;;
      "restore_file")
        dry_run_ui_info "$(_f "TODO: write message - dry_run_restore_file" "$target" "$source")"
        ;;
      *)
        dry_run_ui_info "$(_f "TODO: write message - dry_run_file_operation" "$operation" "$target")"
        ;;
    esac
    return 0
  else
...
dry_run_script_execution() {
  ...
  if is_dry_run; then
    dry_run_ui_info "$(_f "TODO: write message - dry_run_execute_script" "$description")"
    dry_run_ui_info "$(_f "TODO: write message - dry_run_script_path" "$script_path")"
    return 0
  else
    return 1
  fi

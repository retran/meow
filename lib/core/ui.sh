...
indent() {
  ...
  if ! [[ "$level" =~ ^[0-9]+$ ]]; then
    echo -e "\n\e[1;31m[DEBUG] Ошибка вызова функции 'indent':\e[0m" >&2
    echo -e "\e[31m -> Ожидалось число, но получен аргумент: '$1'\e[0m" >&2
    echo -e "\e[31m -> Стек вызовов (кто кого вызвал):\e[0m" >&2
